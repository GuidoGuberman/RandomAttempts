...
START_DIR=/Users/Guido/Documents/Analyses/subjects/PedsData/MNI/"$nummni"/DWI
...
while test $# -gt 0; do
        case "$1" in
                -h|--help)
						echo "This script performs the preprocessing pipeline but takes in arguments to selectively run specific steps. The specific parameters and subjects being run have to be modified in the script itself. Note that the order of the arguments must be followed properly."
                        echo "options:"
                        echo "-i 						base input directory"
                        echo "-o						base output directory"
                        echo "-h, --help                show brief help"
                        echo "-mr       				perform MRConvert step to stride 1,2,3,4"
                        echo "-b1						perform first BET and B0 extraction"
                        echo "-ns						perform nlsam denoising"
                        echo "-n4						perform N4 inhomogeneity correction"
                        echo "-ec						perform Eddy and motion correction"
                        echo "-cu						perform cropping and upsampling to 1x1x1"
                        echo "-dr						perform DTI reconstruction"
                        echo "-ar						perform ANTS SyN registration T1->DWI"
