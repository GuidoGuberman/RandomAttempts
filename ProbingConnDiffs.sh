...
#			cp fodf.nii.gz "$nummni"_run"$numrun"_metrics/fodf.nii.gz
			...
			for parc1rhROIs2 in "${parc1rhROIs[@]}"
			do

				echo "Transforming parcellation label $parc1rhROIs2 to native DWI space, subject $nummni run $numrun"
				cd $SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_diff"$numrun"/"$nummni"_"$numrun"_ROIs/"$nummni"_"$numrun"_ConnectivityProject
				rm "$nummni"_"$parc"_label"$parc1rhROIs2"in_dwi"$numrun".nii
				antsApplyTransforms -d 3 -r $SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_b0c_"$numrun"_1x1x1.nii \
				-t [$SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_diff"$numrun"/"$nummni"_"$numrun"_ROIs/"$nummni"_MNI2DWI"$numrun"_0GenericAffine.mat,0] \
				-n MultiLabel -i $TEMPLATE_DIR/"$parc"_labels/"$parc1rhROIs2".nii* -o "$nummni"_"$parc"_label"$parc1rhROIs2"in_dwi"$numrun".nii -v 1
				...
				"$nummni"_"$parc"_label"$parc1rhROIs2"in_dwi"$numrun".nii --either_end -f
			done

			for parc1rhROIs in "${parc1rhROIs[@]}"
			do
				for roinum2 in "${parc1rhROIs[@]}"
				do
					...
					then
						...
						"$nummni"_"$parc"_label"$roinum2"in_dwi"$numrun".nii --either_end -f
			...
			for parc1lhROIs2 in "${parc1lhROIs[@]}"
			do

				echo "Transforming parcellation label $parc1lhROIs2 to native DWI space, subject $nummni run $numrun"
				cd $SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_diff"$numrun"/"$nummni"_"$numrun"_ROIs/"$nummni"_"$numrun"_ConnectivityProject
				rm "$nummni"_"$parc"_label"$parc1lhROIs2"in_dwi"$numrun".nii
				antsApplyTransforms -d 3 -r $SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_b0c_"$numrun"_1x1x1.nii \
				-t [$SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_diff"$numrun"/"$nummni"_"$numrun"_ROIs/"$nummni"_MNI2DWI"$numrun"_0GenericAffine.mat,0] \
				-n MultiLabel -i $TEMPLATE_DIR/"$parc"_labels/"$parc1lhROIs2".nii* -o "$nummni"_"$parc"_label"$parc1lhROIs2"in_dwi"$numrun".nii -v 1
				...
				"$nummni"_"$parc"_label"$parc1lhROIs2"in_dwi"$numrun".nii --either_end -f
			done

			for parc1lhROIs in "${parc1lhROIs[@]}"
			do
				for roinum3 in "${parc1lhROIs[@]}"
				do
					...
					then
						...
						"$nummni"_"$parc"_label"$roinum3"in_dwi"$numrun".nii --either_end -f
...
#	scil_split_volume_labels.py ../"$parc"_INT32.nii ids
	...
	do
		...
		do
			...
			for parc2rhROIs2 in "${parc2rhROIs[@]}"
			do

				echo "Transforming parcellation label $parc2rhROIs2 to native DWI space, subject $nummni run $numrun"
				cd $SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_diff"$numrun"/"$nummni"_"$numrun"_ROIs/"$nummni"_"$numrun"_ConnectivityProject
				rm "$nummni"_"$parc"_label"$parc2rhROIs2"in_dwi"$numrun".nii
				antsApplyTransforms -d 3 -r $SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_b0c_"$numrun"_1x1x1.nii \
				-t [$SUB_DIR/"$nummni"/DWI/Preproc1/"$nummni"_diff"$numrun"/"$nummni"_"$numrun"_ROIs/"$nummni"_MNI2DWI"$numrun"_0GenericAffine.mat,0] \
				-n MultiLabel -i $TEMPLATE_DIR/"$parc"_labels/"$parc2rhROIs2".nii* -o "$nummni"_"$parc"_label"$parc2rhROIs2"in_dwi"$numrun".nii -v 1
				...
				"$nummni"_"$parc"_label"$parc2rhROIs2"in_dwi"$numrun".nii --either_end -f
			done

			for parc2rhROIs in "${parc2rhROIs[@]}"
			do
				for roinum4 in "${parc2rhROIs[@]}"
				do
					...
					then
						...
						"$nummni"_"$parc"_label"$roinum4"in_dwi"$numrun".nii --either_end -f
